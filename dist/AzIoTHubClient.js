...
exports.ackPayload = exports.AzIoTHubClient = void 0;
...
function generateSasToken(resourceUri, signingKey, policyName, expiresInMins) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const hmac = yield createHmac(signingKey, toSign);
        const base64UriEncoded = encodeURIComponent(hmac);
        let token = "SharedAccessSignature sr=" +
            resourceUri +
            "&sig=" +
            base64UriEncoded +
            "&se=" +
...
class AzIoTHubClient {
    ...
    connect() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (this.modelId)
                userName += `&model-id=${this.modelId}`;
            const password = yield generateSasToken(`${this.host}/devices/${this.deviceId}`, this.key, null, 60);
    ...
    commandResponse(methodName, payload, rid, status) {
        ...
        response.destinationName =
            DIRECT_METHOD_RESPONSE_TOPIC.replace("{status}", status.toString()) +
                rid.toString();
...
exports.AzIoTHubClient = AzIoTHubClient;
